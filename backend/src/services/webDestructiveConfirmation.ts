...
interface Confirmation {
    ...
    action: 'delete_file' | 'delete_storage_account';
    objectId: string;
    expiresAt: number;
}
...
export class WebDestructiveConfirmationStore {
    ...
    issue(input: Omit<Confirmation, 'authTokenHash' | 'expiresAt'> & { authToken: string }) {
    ...
    consume(token: string, input: { authToken: string; action: Confirmation['action']; objectId: string }) {
